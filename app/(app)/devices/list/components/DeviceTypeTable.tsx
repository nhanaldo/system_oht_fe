...
import ModalAddDeviceType from "./ModalAddDeviceType";
import { getDeviceTypes, deleteDeviceType } from "../listAction";
...
export default function DeviceTypeTable() {
    ...
    const { data: rawData, total, refetch, onPageChange, onSearchChange, params } = useTableQuery<DeviceType>({
        queryKey: 'device-types',
        fetchFn: getDeviceTypes,
    ...
    const handleDelete = (id: string) => {
        ...
        const name = item?.name || item?.code || 'nhóm thiết bị này';
    ...
    const handleBatchDelete = () => {
        ...
        openConfirmModal(
            'Thông báo',
            `Bạn có chắc chắn muốn xóa ${selectedRowKeys.length} nhóm thiết bị đã chọn không?`,
            async () => {
                try {
                    const res = await deleteDeviceType(selectedRowKeys as string[]);
                    if (res && res.success === false) {
                        showError(res.error || 'Có lỗi xảy ra khi xóa');
                    } else {
                        showSuccess(`Đã xóa thành công ${selectedRowKeys.length} nhóm thiết bị`);
                        setSelectedRowKeys([]);
                        refetch();
                    }
                } catch {
        ...
        <div className="flex flex-col h-full min-h-0">
            <div className="flex justify-between items-start mb-2 shrink-0">
                <div className="min-w-0 flex-1 mr-2">
                    <h2 className="text-[#373838] font-roboto font-medium leading-none tracking-normal lg:text-[16px] text-[14px] truncate">Quản lý nhóm thiết bị</h2>
